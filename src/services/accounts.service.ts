...
export const AccountsService = {
    ...
    getByProviderAndAccountId: async (provider: string, providerAccountId: string) => {
        try {
            const account = await AccountsRepo.findByProvider(provider, providerAccountId);
            return account;
        } catch (error) {
            throw new HttpError(500, 'Error fetching account by provider');
        }
    }
